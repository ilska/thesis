...
function function_AS1MSR_train_test_data_creation( load_files_path )


save_data_path = strcat(load_files_path,'AS1\train_test_data\');
...
cont7 =1; cont8 = 1; cont9 = 1; cont10 = 1;
 for file = files'
    ...
    [ subject ] = calc_subject_idx_MSR( filename,action );
     ...
     % Action set 1
%      {[2 3 5 6 10 13 18 20]}
    if (action == 2 || action == 3 || action == 5 || action == 6 || action == 10 || action == 13|| action == 18 || action == 20 )
        if (subject == '1')
